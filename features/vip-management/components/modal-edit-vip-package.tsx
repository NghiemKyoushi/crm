...
import { Modal, Button, Input, InputNumber, Form, message } from "antd";
import { useForm, Controller } from "react-hook-form";
import { VipPackage, UpdateVipPackageRequest } from "../apis/vip-api";

const { TextArea } = Input;

interface ModalEditVipPackageProps {
    open: boolean;
    onClose: () => void;
    onSuccess: () => void;
    vipPackage: VipPackage | null;
    onSubmit: (id: number, data: UpdateVipPackageRequest) => Promise<void>;
}

export default function ModalEditVipPackage(props: ModalEditVipPackageProps) {
    const { open, onClose, onSuccess, vipPackage, onSubmit } = props;
    ...
    const {
        ...
        reset,
        formState: { errors },
    } = useForm<UpdateVipPackageRequest>({
        defaultValues: {
            name: "",
            price: 0,
            max_auction_items: 0,
    ...
    useEffect(() => {
        if (open && vipPackage) {
            reset({
                ...
                price: vipPackage.price || 0,
                max_auction_items: vipPackage.max_auction_items || 0,
                cancel_fee: vipPackage.cancel_fee || 0,
                duration_days: vipPackage.duration_days || 0,
                description: vipPackage.description || "",
            });
        }
    }, [open, vipPackage, reset]);

    const onFormSubmit = async (data: UpdateVipPackageRequest) => {
        if (!vipPackage?.id) {
            message.error("Không tìm thấy ID gói VIP");
            return;
        }

        try {
            await onSubmit(vipPackage.id, data);
            ...
            message.error(
                ...
                "Có lỗi xảy ra khi cập nhật gói VIP"
            );
        }
    };
        ...
        <Modal
            title={
                <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                    <span className="font-semibold text-lg">Sửa gói VIP</span>
                </div>
            }
            open={open}
            footer={null}
