...
import { Modal, Input, Radio, Button, Spin } from "antd";
import { useInfiniteQuery } from "@tanstack/react-query";
import { getListStaff } from "@/features/user-management/apis/staff-manage";
...
export default function AddSalesModal({
  ...
  const [selectedId, setSelectedId] = useState<string | null>(null); // ✅ radio chỉ cần 1 id
  const {
    data,
    fetchNextPage,
    hasNextPage,
  ...
  const handleSubmit = () => {
    const selectedEmployees = employees.filter((emp) => emp.id === selectedId);
    onSubmit(selectedEmployees[0]);
    setSelectedId(null);
    setSearch("");
  };
  ...
  return (
    <Modal
      title={
        <span className="font-semibold text-lg">Thay đổi viên phụ trách</span>
      }
      open={open}
      onCancel={onClose}
      footer={null}
